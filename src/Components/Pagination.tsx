...
import { ITEMS_PER_REQUEST } from "../api/api";
...
const Pagination: React.FC<PaginationProps> = ({ currentPage, totalItems, onPageChange }) => {
   const totalPages = Math.ceil(totalItems / ITEMS_PER_REQUEST) - 1; // Calculate total pages from total items
      ...
      if (screenWidth < 550) {
         ...
         if (totalPages > 5 && endPage < totalPages) {
            ...
            pages.push(
               <button
                  key={totalPages}
                  className={`px-3 py-1 rounded ${currentPage === totalPages ? 'bg-[#890A74] text[#FFA900] font-medium' : 'bg-gray-200 text-gray-700'} 
                        hover:bg-[rgba(137,10,116,0.45)] hover:text-[rgba(255,169,25)] hover:cursor-pointer`}
