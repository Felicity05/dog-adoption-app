...
const Match: React.FC = () => {
    ...
    return (
        <div className="p-4 mt-6 text-center flex flex-col items-center">
            ...
            {match && showMatch && (
                <div className="mt-5 bg-white-400 p-4 rounded-lg shadow-lg relative w-[300px]">
                    <button
                        ...
                        className="absolute top-2 right-2 p-1 rounded-full bg-[#890A74] text-[#FFA900]
                                    hover:bg-[#FFA900] hover:text-[#890A74] 
                                     shadow-md"
                    >
                        <XIcon className="hover:animate-[spin_1s_ease-in-out]"/>
                    </button>
                    <h3 className="text-xl font-semibold text-[#890A74] ">{match.name}</h3>
                    <img src={match.img} alt={match.name} className="w-full h-48! object-cover rounded-md mt-2" />
                </div>
            )}
